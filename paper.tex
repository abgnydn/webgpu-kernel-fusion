...
\begin{abstract}
Fusing sequential fitness evaluations into single GPU compute shader dispatches eliminates the per-step kernel launch overhead that dominates framework-based GPU computation. On the same Apple M2 Pro hardware, a hand-fused WebGPU compute shader achieves 159$\times$ over PyTorch MPS on a 1,500-timestep financial simulation (46.2 vs 0.29~gen/s) while matching PyTorch on embarrassingly parallel workloads (1.06$\times$), isolating the advantage to sequential dispatch overhead. An unfused ablation (500 separate dispatches vs 1) yields a clean 2.18$\times$ from fusion alone. Cross-hardware, JAX GPU with \texttt{lax.scan}+\texttt{vmap} on Tesla T4 achieves 6.43~gen/s on the financial simulation --- 13$\times$ over PyTorch CUDA on the same T4, confirming fusion helps, but still 7.2$\times$ slower than WebGPU. The gap narrows to 1.29$\times$ at $L=500$ (Acrobot), revealing the advantage scales with episode length. We show \texttt{torch.compile} fails at $L \geq 1{,}000$ and that WebGPU dominates CMA-ES across all dimensionality regimes. A native Metal baseline via wgpu quantifies Chrome's browser overhead at 1.92$\times$. The insight --- hand-fused shaders outperform framework dispatch on sequential fitness functions --- applies beyond WebGPU, and WebGPU makes such fusion zero-install.
\end{abstract}
...
\textbf{Contributions:}
\begin{itemize}
  \item \textbf{C1: Kernel fusion for sequential fitness functions.} On the same M2 Pro GPU, fusing a 1,500-timestep financial simulation yields 159$\times$ over PyTorch MPS. On the same T4 GPU, JAX with \texttt{lax.scan}+\texttt{vmap} achieves 13$\times$ over PyTorch CUDA via fusion but remains 7.2$\times$ slower than WebGPU. An unfused ablation isolates 2.18$\times$ from fusion alone.
...
\textbf{Same-hardware (M2 Pro):} WebGPU fused achieves 54$\times$ over PyTorch MPS. \textbf{Same-hardware (T4):} JAX with fusion achieves 172$\times$ over PyTorch CUDA. \textbf{Cross-hardware numbers (223$\times$) include M2 Pro's unified memory advantage and should not be interpreted as pure fusion gains.}

\textbf{JAX GPU nearly closes the gap at $L=500$.} JAX achieves 105.1~gen/s --- only 1.29$\times$ slower than WebGPU (cross-hardware caveat applies). Combined with the financial result (7.2$\times$ gap at $L=1{,}500$), this suggests the fusion advantage scales with episode length $L$.

\textbf{Ablation (same hardware, M2 Pro).} Unfused WebGPU (500 separate dispatches) achieves 62.3~gen/s; fused achieves 135.9~gen/s --- a clean \textbf{2.18$\times$ from fusion alone}, with no hardware variable. JAX's fusion (105.1, T4) falls between unfused and fused WebGPU, though direct comparison is confounded by hardware differences.
...
Hand-fused compute shaders outperform framework-based dispatch on sequential fitness functions. On the same M2 Pro hardware, WebGPU achieves 159$\times$ over PyTorch MPS on a 1,500-timestep financial simulation while matching it on parallel workloads (1.06$\times$), isolating the advantage to sequential dispatch overhead. An unfused ablation yields a clean 2.18$\times$ from fusion alone. On the same T4 hardware, JAX's XLA fusion achieves 13$\times$ over PyTorch CUDA --- confirming fusion helps --- but cross-hardware comparison with WebGPU (7.2$\times$) is confounded by M2 Pro's unified memory advantage and should be interpreted cautiously.
